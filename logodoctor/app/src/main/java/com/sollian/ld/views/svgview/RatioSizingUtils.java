package com.sollian.ld.views.svgview;
...
public class RatioSizingUtils {
    ...
    public static RatioSizingInfo getRatioSizingInfoFromAttrs(Context context,
            AttributeSet attrs) throws IllegalArgumentException {
        ...
        if (attrs != null) {
            ...
            String ratioString = a
                    .getString(R.styleable.AspectRatioLayout_oakAspectRatio);
            try {
    ...
    public static RatioSizingInfo parseRatioSizingInfo(String ratioString)
    ...
    public static RatioMeasureInfo getMeasureInfo(int widthMeasureSpec,
            int heightMeasureSpec, RatioSizingInfo ratioSizingInfo,
        ...
        rmi.height = View.MeasureSpec.getSize(heightMeasureSpec)
                - heightPadding;
        int widthMode = View.MeasureSpec.getMode(widthMeasureSpec);
        int heightMode = View.MeasureSpec.getMode(heightMeasureSpec);

        if (rmi.height <= 0 && rmi.width <= 0
                && heightMode == View.MeasureSpec.UNSPECIFIED
                && widthMode == View.MeasureSpec.UNSPECIFIED) {
            rmi.width = 0;
            rmi.height = 0;
        } else if (rmi.height <= 0
                && heightMode == View.MeasureSpec.UNSPECIFIED) {
        ...
        } else if (rmi.width * ratioSizingInfo.aspectRatioHeight > ratioSizingInfo.aspectRatioWidth
                * rmi.height) {
            rmi.width = (int) (rmi.height * ratioSizingInfo.aspectRatioWidth / ratioSizingInfo.aspectRatioHeight);
        } else {
            rmi.height = (int) (rmi.width * ratioSizingInfo.aspectRatioHeight / ratioSizingInfo.aspectRatioWidth);
